...
def logic(new_price, macd_change, cash_allocation, coin_allocation, cash_balance, coin_balance, simulation):
...
def update(symbol):
    ...
    new_macd = new_data['macd'][len(new_data)-1]
    ...
    for i in range(len(trials)):
        trial = trials[i]
        print(trial)
        simulation = pd.read_csv('../storage/crypto/BINANCE/tables/current/simulations/macd/long/absolute/slope/' + symbol + '/' + trial)
        if str(simulation['Time'][len(simulation)-1]) == 'x':
            ...
            simulation['MACD'][len(simulation)-1] = new_macd
            simulation['MACD Change'][len(simulation)-1] = 0
            ...
            print(simulation)
            simulation.to_csv('../storage/crypto/BINANCE/tables/current/simulations/macd/long/absolute/slope/' + symbol + '/' + trial, index=False)
         
        else:
            print('not initial run')
            
            price_change = new_price - simulation['Price'][len(simulation)-1]
            macd_change = new_macd - simulation['MACD'][len(simulation)-1]
            ...
            cash_allocation, coin_allocation, cash_balance, coin_balance = logic(new_price, macd_change, cash_allocation, coin_allocation, cash_balance, coin_balance, simulation)
