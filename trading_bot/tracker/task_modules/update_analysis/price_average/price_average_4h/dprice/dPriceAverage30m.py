...
import pandas as pd
import numpy as np
from .dprice_average_30m import ddPrice

CSV_PATH = '../storage/crypto/BINANCE/tables/current/analysis/price_average/price_average_4h/dprice/dprice_average_30m.csv'
RAW_PATH = '../storage/crypto/BINANCE/tables/current/analysis/price_average/price_average_4h/dprice.csv'
dt = 30

def updateSelf(symbols):
    Misc.printDebug('dPriceAverage30m.updateSelf(): STARTED')
    ...
    #print(df)
    Misc.printDebug('dPriceAverage30m.updateSelf(): FINISHED')

def update(symbols):
    Misc.printDebug('dPriceAverage30m.update(): STARTED')

    updateSelf(symbols)
    ddPrice.update(symbols)

    Misc.printDebug('dPriceAverage30m.update(): FINISHED')
