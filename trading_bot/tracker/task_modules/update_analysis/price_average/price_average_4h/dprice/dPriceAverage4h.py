...
import pandas as pd
import numpy as np
from .dprice_average_4h import ddPrice

CSV_PATH = '../storage/crypto/BINANCE/tables/current/analysis/price_average/price_average_4h/dprice/dprice_average_4h.csv'
RAW_PATH = '../storage/crypto/BINANCE/tables/current/analysis/price_average/price_average_4h/dprice.csv'
dt = 4*60

def updateSelf(symbols):
    Misc.printDebug('dPriceAverage4h.updateSelf(): STARTED')
    ...
    #print(df)
    Misc.printDebug('dPriceAverage4h.updateSelf(): FINISHED')

def update(symbols):
    Misc.printDebug('dPriceAverage4h.update(): STARTED')

    updateSelf(symbols)
    ddPrice.update(symbols)

    Misc.printDebug('dPriceAverage4h.update(): FINISHED')
