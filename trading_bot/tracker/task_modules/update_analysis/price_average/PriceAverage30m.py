...
import pandas as pd
import numpy as np
from .price_average_30m import dPrice

CSV_PATH = '../storage/crypto/BINANCE/tables/current/analysis/price_average/price_average_30m.csv'
...
def updateSelf(symbols):
    Misc.printDebug('PriceAverage30m.updateSelf(): STARTED')
    ...
    Misc.printDebug('PriceAverage30m.updateSelf(): FINISHED')

def update(symbols):
    Misc.printDebug('PriceAverage30m.update(): STARTED')

    updateSelf(symbols)
    dPrice.update(symbols)

    Misc.printDebug('PriceAverage30m.update(): FINISHED')
